...
top_species <- merged_relab_df |>
  ...
  dplyr::filter(Species != "uncultured_bacterium") |>
...
color_map <- setNames(wheel_colors, levels(merged_df$Species))
...
g <- ggplot(alpha_div_df, aes(x=status_Sample_type, y=Alpha)) + 
...
```{r Alpha Diversity}
p2_1 <- plot_richness(ps_ms, measures=c("Observed", "Shannon", "Simpson", "InvSimpson"),
                      x = c("status")) + 
...
ggarrange(p2_1, p2_2, p2_3, p2_4, p2_5, p2_6)
```
