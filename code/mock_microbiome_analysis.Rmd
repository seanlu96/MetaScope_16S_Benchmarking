...
```{r Summarize MetaScope Results, eval=FALSE, echo=FALSE}
...
```{R}
results_priors_metaid <- create_summary_df(
  list.files(
    path = "data_processed/kozich_2013/results_metablast", 
    pattern = ".metascope_id.csv", 
    full.names = TRUE, 
    recursive = TRUE), 
  pipeline_name = "MetaScope"
)
...
create_summary_df <- function(csv_paths, pipeline_name) {
  ...
  for (i in csv_paths) {
    ...
    head(metascope_df)
    metascope_df <- metascope_df |>
      dplyr::select(species, reassigned_read_count) |>
      dplyr::mutate_if(is.numeric, ~ . / sum(.))
    colnames(metascope_df) <- c("Species", sample_name)
    #metascope_df[metascope_df == "Phocaeicola vulgatus"] <- 'Bacteroides vulgatus'
    #metascope_df[metascope_df == "Schaalia odontolytica"] <- 'Actinomyces odontolyticus'
...
results_priors_metablast <- create_summary_df(
  list.files(
    path = "data_processed/kozich_2013/results_metablast", 
    pattern = ".metascope_blast_reassigned.csv", 
    full.names = TRUE, 
    recursive = TRUE), 
  pipeline_name = "MetaBlast"
)


summary_df_final_3 <- rbind(results_priors_metaid, results_priors_metablast)

summary_df_final_3 |> dplyr::filter(Species %in% c("No Call", "Incorrect Call")) |>
  tidyr::pivot_wider(names_from = Species, values_from = prop, names_repair = "universal") |>
  dplyr::mutate(Correct.Call = 1 - (No.Call + Incorrect.Call)) |>
  dplyr::filter(pipeline != "Ground Truth") |>
  dplyr::group_by(pipeline) |>
  ...
  xlab("Sum of total priors") + 
  ylab("Percentage of Correct Calls") +
  stat_compare_means(comparisons = list(c("MetaScope", "MetaBlast")), 
                     method = "t.test", 
                     paired = TRUE) + 
  geom_line(aes(group = name))
  
dplyr::left_join(results_priors_metaid, results_priors_metablast, by = c("Species","name")) |>
  dplyr::filter(Species == "Incorrect Call") |> 
  dplyr::mutate(diff = prop.x - prop.y) |>
  ggplot(aes(y = diff, x = name)) +
  geom_point() + 
...
results_priors_metaid_5 <- create_summary_df(
  list.files(
    path = "data_processed/kozich_2013/results_metablast_bowtie_5", 
    pattern = ".metascope_id.csv", 
    full.names = TRUE, 
    recursive = TRUE), 
  pipeline_name = "MetaScope"
...
results_priors_metablast_5 <- create_summary_df(
  list.files(
    path = "data_processed/kozich_2013/results_metablast_bowtie_5", 
    pattern = ".metascope_blast_reassigned.csv", 
    full.names = TRUE, 
...
summary_df_final_4 <- rbind(results_priors_metaid_5, results_priors_metablast_5)

summary_df_final_4 |> dplyr::filter(Species %in% c("No Call", "Incorrect Call")) |>
  tidyr::pivot_wider(names_from = Species, values_from = prop, names_repair = "universal") |>
  dplyr::mutate(Correct.Call = 1 - (No.Call + Incorrect.Call)) |>
  dplyr::filter(pipeline != "Ground Truth") |>
  dplyr::group_by(pipeline) |>
...
dplyr::left_join(results_priors_metaid_5, results_priors_metablast_5, by = c("Species","name")) |>
  dplyr::filter(Species == "Incorrect Call") |> 
  dplyr::mutate(diff = prop.x - prop.y) |>
  ggplot(aes(y = diff, x = name)) +
  geom_point() + 
