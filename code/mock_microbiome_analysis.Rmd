...
dada2_tax <- dada2_df |>
  ...
  rename(Kingdom = "domain", Phylum = "phylum", Class = "class", Order = "order",
         Family = "family", Genus = "genus", Species = "species")

dada2_otu <- dada2_df |>
  select(taxid, reads_count, name) |>
  mutate(dada2_name = paste0(name, ".dada2")) |>
  select(-name) |>
  group_by(taxid,dada2_name) |>
  summarise(value = sum(reads_count)) |> 
  pivot_wider(names_from = dada2_name, values_from = value) |>
  mutate(across(everything(), ~ replace_na(.x, 0)))
